...
client.on('ready', () => {
    client.user.setActivity('Subscribe PenomStudio', { type: 'WATCHING' });
    console.clear();
 
  console.log('\n\x1b[32m%s\x1b[0m', `          $[INFO]: Logged into ${client.user.tag}`);  
  console.log('\x1b[32m%s\x1b[0m', `           $[INFO]: PREFIX ${mainprefix}`);  
  console.log('\x1b[32m%s\x1b[0m', `           $[EXTRA]: CUSTOM MADE ANTI-RAID`);  

   console.log('\x1b[31m%s\x1b[0m', `            $[CREDITS]: Made By PenomStudio`);  
  console.log('\x1b[31m%s\x1b[0m', `            $[SUPPORT]: Source Code: https://discord.gg/TsnkNntndQ `);  
 ...
 client.on("message", async message => {
   ...
   if(prefix === null) prefix = mainprefix;
      ...
      if(premiumcheck && premiumcheck.find(find => find.kid == message.author.id)) return message.channel.send(`<:ModMute:828743734727278632> | You are blacklisted from using this bot!`);
...
db.delete(`executer_${role.guild.id}_${entry.id}`)
 ...
 .setTitle(`${entry.tag} was trying to raid but failed miserabely! [Breaking Roles Create Limits]`)
